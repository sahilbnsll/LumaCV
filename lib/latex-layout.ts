...
export const THRESHOLDS: Record<TemplateType, { tight: number; overflow: number }> = {
    modern: { tight: 3800, overflow: 5600 },
    classic: { tight: 3600, overflow: 5400 },
    ats: { tight: 4000, overflow: 6000 },
    executive: { tight: 3200, overflow: 5000 },
    minimal: { tight: 2800, overflow: 4600 },
    compact: { tight: 4800, overflow: 7600 },
    creative: { tight: 3200, overflow: 5000 },
    tech: { tight: 3600, overflow: 5600 },
};
...
export function getTemplateFitLevel(data: ResumeData, template: TemplateType): TemplateFitLevel {
    ...
    if (template === 'minimal') return volume < tight * 0.72 && sections <= 7 ? 'recommended' : 'good';
    if (template === 'ats' || template === 'classic') return sections >= 6 ? 'recommended' : 'good';
    if (template === 'tech') return (data.skills?.length ?? 0) >= 6 ? 'recommended' : 'good';
    if (template === 'creative') return sections <= 6 ? 'good' : 'tight';
