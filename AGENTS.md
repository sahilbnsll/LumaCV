...
- `lib/resume-export.ts`'s PDF export must not fabricate a fallback file when `/api/v1/resume/compile` fails. It used to return a "PDF" that was really markdown text with a `%PDF-1.5` string prepended, not a valid PDF by any real structure, while reporting success, so a real compile failure silently handed the user a corrupt download with no error. If a format's real generation path fails, surface `notify.error()` with whatever detail is available and return `false`, never synthesize a fake file to paper over it.
- The Word export (`lib/docx-generator.ts`, via the `docx` npm package) builds a genuine OOXML `.docx`, not an HTML file wearing a Word extension. An earlier version used Word's legacy HTML-flavored trick and saved it as `.docx` anyway, modern Word validates the actual content against the extension and refuses to open it. If you ever go back to an HTML-based export for any reason, the extension and MIME type must be `.doc` / `application/msword` to match what's actually being generated, never `.docx` / the OOXML MIME type for non-OOXML content.
- `lib/typst-generator.ts`'s achievement/publication field mapping uses `safeTrim()`, not `(a || b || '').trim()`. An empty array or object is truthy in JS, so that pattern doesn't reliably fall through to `''` the way it looks like it should, a malformed AI-tailored field (an array where a string belongs) reached `.trim()` on a non-string and crashed the entire export with no useful error. Keep using `safeTrim()` (or an equivalent explicit `typeof === 'string'` check) for any field whose value might not be trustworthy.
