// ============================================================
// Template: Forma — Asymmetric Left-Label Architectural Layout
// ============================================================
// Design principles:
//  - Bold 3pt accent rule beneath prominent header
//  - Asymmetric 2-column grid: section titles on left (0.95fr), content on right (4.4fr)
//  - Burnt orange accent, crisp dark ink item titles, complete data fidelity
...
#let render-forma(data, variant: "default", theme: "forma") = {
  let default-palette = (
    ...
    marker: rgb("#c2410c"),
    link: rgb("#c2410c"),
    name-fill: rgb("#15191d"),
    headline-fill: rgb("#c2410c"),
    company-fill: rgb("#c2410c"),
  )
  ...
  let accent-bg = if "accent" in t { t.accent.lighten(94%) } else { rgb("#fff7ed") }
  let accent-border = if "accent" in t { t.accent.lighten(70%) } else { rgb("#fed7aa") }
  ...
  grid(
    ...
    align(right)[
      ...
      #set text(size: 7.7pt, fill: t.muted)
      #{
  ...
  v(0.25em)
  block(fill: t.accent, width: 100%, height: 2.8pt)[ ]

  let section(title, body, breakable: true) = {
    v(1.25em)
    block(width: 100%, breakable: breakable, {
      grid(
        columns: (1.10fr, 4.15fr),
        column-gutter: 1.0em,
        align: (left + top, left + top),
        [
          #block(above: 0pt, below: 0pt, breakable: false, sticky: true, width: 100%, {
            text(size: 8.5pt, weight: "bold", tracking: 0.10em, fill: t.accent)[#heading(level: 2, outlined: true, bookmarked: true, upper(title))]
            v(0.08em)
            line(length: 22pt, stroke: 1.2pt + t.accent)
          })
        ],
        body,
      )
  ...
  for sec in active-order {
    if sec == "summary" {
      ...
      if "techStackSummary" in data and data.techStackSummary != none and data.techStackSummary != "" {
        section("Tech Stack", block(
          fill: accent-bg,
          stroke: 0.4pt + accent-border,
          inset: (x: 5pt, y: 3pt),
          radius: 2pt,
          width: 100%,
          text(size: 8.1pt)[#text(weight: "bold", fill: t.ink)[Architecture & Tools: ] #parse-bold(data.techStackSummary)]
        ))
      }
        ...
        section("Key Metrics", {
          ...
          grid(
            ...
            ..data.keyMetrics.map(km => block(
              ...
              width: 100%,
              align(center)[
                #text(size: 9.3pt, weight: "bold", fill: t.accent)[#km.value] \
                #v(0.04em)
          ...
          for (i, exp) in data.experience.enumerate() {
            ...
            block(width: 100%, breakable: false, {
              grid(
                ...
                text(size: 7.9pt, fill: t.muted)[#exp.dates],
              )
              v(0.02em)
          ...
          for (i, edu) in data.education.enumerate() {
            ...
            block(width: 100%, breakable: false, {
              grid(
                ...
                text(size: 7.9pt, fill: t.muted)[#edu.dates],
              )
              v(0.02em)
              grid(
                columns: (1fr, auto),
                [#text(size: 8.1pt, fill: t.company-fill)[#edu.degree#if "specialization" in edu and edu.specialization != "" [ · #edu.specialization]]],
                if "gpa" in edu and edu.gpa != "" { text(size: 7.7pt, fill: t.muted)[GPA: #edu.gpa] }
              )
            })
          }
        }, breakable: false)
            ...
            block(width: 100%, breakable: false, {
              grid(
                ...
                [
                  ...
                  #if "role" in proj and proj.role != "" [ #text(fill: t.muted)[·] #text(size: 7.8pt, style: "italic", fill: t.muted)[#proj.role] ]
          ...
          for (i, pat) in data.patents.enumerate() {
            ...
            grid(
              ...
              [
                ...
                #if meta.len() > 0 [ #text(size: 7.7pt, fill: t.muted)[· #meta.join(", ")] ]
              ],
              text(size: 7.8pt, fill: t.muted)[#if "date" in pat { pat.date }]
            )
            if "description" in pat and pat.description != "" {
              v(0.04em)
              text(size: 7.9pt, fill: t.ink)[#parse-bold(pat.description)]
            }
          }
        }, breakable: false)
      }
        ...
        section("Products", {
          for (i, prod) in data.products.enumerate() {
            ...
            if "description" in prod and prod.description != "" {
              v(0.04em)
              text(size: 7.9pt, fill: t.ink)[#parse-bold(prod.description)]
            }
          }
        }, breakable: false)
      }
      ...
      if "additionalInfo" in data and data.additionalInfo != none {
        ...
        if items.len() > 0 {
          section("Additional Info", items.join(text(fill: t.muted)[ · ]), breakable: false)
        }
      }
