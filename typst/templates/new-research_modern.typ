...
#import "/lib/theme.typ": font-serif, font-sans, colors, typo, gap, pg, render-skills-adaptive, render-footer
...
#let render-research_modern(data, variant: "default", theme: "research_modern") = {
  let t = (
    ink: rgb("#1c2228"),
    accent: rgb("#1b365d"),       // Deep lab navy
    accent-light: rgb("#edf3f8"), // Soft scientific tint
    muted: rgb("#5a6572"),
    marker: rgb("#1b365d"),
    rule: rgb("#1b365d").lighten(65%),
    tag-bg: rgb("#f3f6f9"),
    tag-border: rgb("#dce4ec"),
  )
  ...
  set document(title: name + " — Scientific CV", author: name, date: none)
  set page(
    paper: "a4",
    margin: (x: 1.55cm, top: 1.40cm, bottom: 1.65cm),
  ...
  // -------------------------------------------------------------
  align(center)[
    #text(size: 21pt, weight: "bold", fill: t.accent)[#name]
    #if headline != "" [
      #v(0.20em)
      #text(size: 10pt, style: "italic", fill: t.muted)[#headline]
    ]
    #v(0.35em)
    #{
      ...
      text(size: 8pt, fill: t.muted)[#items.join(text(fill: t.accent)[ · ])]
    }
    #v(0.40em)
  ...
  let section(title, body, breakable: true) = {
    ...
    block(width: 100%, breakable: breakable, {
      block(above: 0pt, below: 0.50em, breakable: false, sticky: true, width: 100%, {
        stack(
          spacing: 0.28em,
          text(size: 9.5pt, weight: "bold", fill: t.accent, font: font-sans)[#upper(title)],
          line(length: 100%, stroke: 0.5pt + t.accent.lighten(35%))
        )
      })
      body
    })
  ...
  for sec in active-order {
    if sec == "summary" {
      if summary != "" {
        section("Research & Professional Profile", [
          #set text(size: 8.8pt)
          #parse-bold(summary)
        ])
      }
      ...
      if "techStackSummary" in data and data.techStackSummary != "" {
        section("Core Research & Technical Stack", [
          #rect(
            width: 100%,
            fill: t.tag-bg,
      ...
      if skills != () and skills.len() > 0 {
        section("Research Competencies & Methodologies", render-skills-adaptive(t, skills, max-categories-for-grid: 4))
      }
    } else if sec == "keyMetrics" {
      if "keyMetrics" in data and data.keyMetrics.len() > 0 {
        section("Quantitative Impact & Key Metrics", {
      ...
      if "experience" in data and data.experience.len() > 0 {
        section("Research & Professional Experience", breakable: true, {
              ...
              #grid(
                ...
                [
                  #text(weight: "bold", size: 9.3pt, fill: t.ink)[#exp.role]
                    ...
                    #text(fill: t.accent)[ · #exp.company]
                  ]
      ...
      if "internships" in data and data.internships.len() > 0 {
        section("Research Fellowships & Internships", breakable: false, {
      ...
      if projects != () and projects.len() > 0 {
        section("Research Projects & Scientific Systems", breakable: false, {
      ...
      if "certifications" in data and data.certifications.len() > 0 {
        section("Certifications & Professional Licensure", {
      ...
      if not awards-rendered {
        ...
        if awards-data.len() > 0 {
          ...
          section("Honors, Grants & Scientific Awards", {
      ...
      if "openSource" in data and data.openSource.len() > 0 {
        section("Scientific & Open-Source Software", render-open-source(data, t.accent))
      }
    } else if sec == "publications" {
      if "publications" in data and data.publications.len() > 0 {
        section("Publications & Working Papers", render-publications(data, t.accent))
      }
    } else if sec == "leadership" {
      if "leadership" in data and data.leadership.len() > 0 {
        section("Academic & Scientific Leadership", render-leadership(data, t.accent))
      }
    } else if sec == "volunteering" {
      if "volunteering" in data and data.volunteering.len() > 0 {
        section("Service & Community Outreach", render-volunteering(data, t.accent))
      }
    } else if sec == "conferences" {
      if "conferences" in data and data.conferences.len() > 0 {
        section("Conferences, Symposia & Invited Talks", render-conferences(data, accent: t.accent))
      }
      ...
      if "interests" in data and data.interests.len() > 0 {
        section("Research Interests & Activities", render-interests(data))
      }
    } else if sec == "patents" {
      ...
      if "products" in data and data.products.len() > 0 {
        section("Products & Scientific Systems", {
      ...
      if "devopsContributions" in data and data.devopsContributions.len() > 0 {
        section("Infrastructure & Laboratory Tooling", {
      ...
      if "securityContributions" in data and data.securityContributions.len() > 0 {
        section("Security & Data Integrity Contributions", {
