// ============================================================
// Template: Nordic — Minimalist Clean Teal & Sage Architecture
// ============================================================
...
#let render-nordic(data, variant: "default", theme: "nordic") = {
  let default-palette = (
    ...
    accent: rgb("#2c6356"),
    soft: rgb("#f1f6f4"),
    stroke-line: rgb("#d3e0db"),
    marker: rgb("#2c6356"),
    link: rgb("#2c6356"),
    name-fill: rgb("#1f2a30"),
    headline-fill: rgb("#2c6356"),
    company-fill: rgb("#2c6356"),
  )
  ...
  // Header Block (Nordic Minimalist Architecture)
  block(width: 100%, breakable: false, {
    text(size: 23pt, weight: "bold", fill: ink, tracking: -0.01em)[#name]
    if headline != "" {
      v(0.04em)
      text(size: 9.8pt, weight: "medium", fill: accent)[#headline]
    }
    v(0.18em)
    text(size: 8.0pt, fill: muted)[#contact-items.join([ #text(fill: stroke-line)[·] ])]
    v(0.24em)
    line(length: 100%, stroke: 0.9pt + stroke-line)
  })
  ...
  let section(title, body, breakable: true) = {
    ...
    block(width: 100%, breakable: breakable, {
      block(above: 0pt, below: 0.50em, breakable: false, sticky: true, width: 100%, {
        stack(
          spacing: 0.28em,
          text(size: 8.8pt, weight: "bold", fill: accent, tracking: 0.16em)[#heading(level: 2, outlined: true, bookmarked: true, upper(title))],
          line(length: 36pt, stroke: 1.5pt + accent)
        )
      })
      body
    })
