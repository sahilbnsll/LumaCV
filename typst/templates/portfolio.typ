...
#let render-portfolio(data, variant: "default", theme: "portfolio") = {
  ...
  let section(title, body) = {
    ...
    block(above: 0pt, below: 0.13em, breakable: false, sticky: true, width: 100%, {
      text(size: 9.3pt, weight: "bold", fill: accent, tracking: 0.12em)[#upper(title)]; v(0.08em); line(length: 22pt, stroke: 1.8pt + accent)
    })
    body
  }
  text(size: 25pt, weight: "bold", fill: ink)[#name] v(0.05em) if headline != "" { text(size: 9.3pt, fill: accent)[#headline] } v(0.15em) block(fill: soft, inset: (x: 0.45em, y: 0.30em), radius: 5pt, [#text(size: 7.7pt)[#contact-text]]) v(0.18em)
    ...
    section("Experience", {
      for (i, exp) in data.experience.enumerate() {
        ...
        block(width: 100%, breakable: false, {
          grid(columns: (1fr, auto), text(size: 9.1pt, weight: "bold")[#exp.role] #text(fill: accent)[@] #text(weight: "bold")[#exp.company], text(size: 8pt, fill: muted)[#exp.dates]); if "location" in exp and exp.location != "" { v(0.03em); text(size: 7.9pt, fill: muted)[#exp.location] }
          v(0.16em)
  ...
  if projects != () and projects.len() > 0 {
    section("Projects", {
      for (i, proj) in projects.enumerate() {
        if i > 0 { v(0.42em) }
        block(fill: soft, inset: (x: 0.50em, y: 0.36em), radius: 5pt, width: 100%, [#grid(columns: (1fr, auto), #text(weight: "bold")[#proj.name], #if "stack" in proj { #text(size: 7.7pt, fill: accent)[#proj.stack] } else { none }) #v(0.08em) #if "description" in proj [#parse-bold(proj.description)]])
      }
    })
  }
  ...
  if "education" in data and data.education.len() > 0 {
    section("Education", {
      for (i, edu) in data.education.enumerate() {
        if i > 0 { v(0.34em) }
        grid(columns: (1fr, auto), text(weight: "bold")[#edu.degree], text(size: 8pt, fill: muted)[#edu.dates]); v(0.03em); text(size: 8.1pt, fill: accent)[#edu.institution]
      }
    })
  }
