...
#let render-monochrome(data, variant: "default", theme: "monochrome") = {
  ...
  let section(title, body) = {
    ...
    block(above: 0pt, below: 0.13em, breakable: false, sticky: true, width: 100%, {
      block(fill: ink, inset: (x: 0.34em, y: 0.16em), width: 100%, [#text(size: 8.4pt, weight: "bold", fill: white, tracking: 0.10em)[#upper(title)]])
    })
    body
  }
  grid(columns: (1fr, auto), text(size: 22pt, weight: "bold")[#upper(name)], align(right)[text(size: 7.7pt, fill: muted)[#contact-text]]); v(0.08em); if headline != "" { text(size: 9pt, weight: "bold")[#headline] }; v(0.15em); line(length: 100%, stroke: 2pt + ink)
    ...
    section("Experience", {
      for (i, exp) in data.experience.enumerate() {
        ...
        block(width: 100%, breakable: false, {
          grid(columns: (1fr, auto), text(size: 9pt, weight: "bold")[#exp.company], text(size: 7.9pt, fill: muted)[#exp.dates]); v(0.03em); text(size: 8.3pt, weight: "bold")[#exp.role#if "location" in exp and exp.location != "" [ · #text(fill: muted)[#exp.location]]]
          v(0.16em)
  ...
  if projects != () and projects.len() > 0 {
    section("Projects", {
      for (i, proj) in projects.enumerate() {
        if i > 0 { v(0.42em) }
        text(weight: "bold")[#proj.name] #if "stack" in proj [#text(size: 7.7pt, fill: muted)[· #proj.stack]] #if "description" in proj [— #parse-bold(proj.description)]
      }
    })
  }
  ...
  if "education" in data and data.education.len() > 0 {
    section("Education", {
      for (i, edu) in data.education.enumerate() {
        if i > 0 { v(0.34em) }
        grid(columns: (1fr, auto), text(weight: "bold")[#edu.institution], text(size: 7.9pt, fill: muted)[#edu.dates]); v(0.03em); text(size: 8.1pt)[#edu.degree#if "specialization" in edu [ · #edu.specialization]]
      }
    })
  }
