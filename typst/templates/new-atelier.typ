// ============================================================
// Template: Atelier — Warm Linen & Earthy Artisan Layout
// ============================================================
// Design principles:
//  - Warm parchment-tinted header card (#f6f1ec) with saddle brown accent (#946844)
//  - Deep espresso ink (#2c2723), warm linen border rules (#ddd3c9)
//  - High-contrast typography: bold ochre section titles, dark espresso item titles, warm muted metadata
//  - Guaranteed vertical separation with stack() to prevent line overlap
...
#let render-atelier(data, variant: "default", theme: "atelier") = {
  let default-palette = (
    ...
    muted: rgb("#726860"),
    soft: rgb("#f6f1ec"),
    stroke-line: rgb("#ddd3c9"),
    marker: rgb("#946844"),
    link: rgb("#946844"),
    name-fill: rgb("#2c2723"),
    headline-fill: rgb("#946844"),
    company-fill: rgb("#946844"),
  )
  ...
  set page(paper: pg.paper, margin: (x: 1.25cm, top: 1.35cm, bottom: 1.65cm), footer: render-footer(t, name))
  ...
  // Header Block (Atelier Linen Card)
  block(
    width: 100%,
    fill: soft,
  ...
  let section(title, body, breakable: true) = {
    ...
    block(width: 100%, breakable: breakable, {
      block(above: 0pt, below: 0.50em, breakable: false, sticky: true, width: 100%, {
        stack(
          spacing: 0.32em,
          text(size: 8.8pt, weight: "bold", tracking: 0.12em, fill: accent)[#heading(level: 2, outlined: true, bookmarked: true, upper(title))],
  ...
  for sec in active-order {
    if sec == "summary" {
      if summary != "" { section("Artisan Profile", parse-bold(summary)) }
      ...
      if "keyMetrics" in data and data.keyMetrics != none and data.keyMetrics.len() > 0 {
        section("Scale & Organizational Impact", breakable: false, {
      ...
      if projects != () and projects.len() > 0 {
        section("Crafted Projects & Architecture", breakable: false, {
