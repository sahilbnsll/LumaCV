...
#let render-timeline(data, variant: "default", theme: "timeline") = {
  ...
  let section(title, body) = {
    ...
    block(above: 0pt, below: 0.13em, breakable: false, sticky: true, width: 100%, {
      text(size: 9.3pt, weight: "bold", fill: accent, tracking: 0.16em)[#upper(title)]; v(0.09em); line(length: 100%, stroke: 0.5pt + stroke-line)
    })
    body
  }
  grid(columns: (1fr, auto), align: (left + top, right + top), text(size: 22pt, weight: "bold")[#name], [#if headline != "" [#text(size: 8.8pt, fill: accent)[#headline]]
  v(0.04em)#text(size: 7.7pt, fill: muted)[#contact-text]]); v(0.22em); line(length: 100%, stroke: 1.0pt + accent)
    ...
    section("Experience", {
      for (i, exp) in data.experience.enumerate() {
        ...
        block(width: 100%, breakable: false, {
          grid(columns: (1.15fr, 0.10fr, 4.85fr), column-gutter: 0.52em, [#text(size: 7.9pt, fill: muted)[#exp.dates]], [#line(length: 100%, stroke: 1.0pt + accent)], [#text(size: 9pt, weight: "bold")[#exp.role] v(0.02em) #text(size: 8.2pt, weight: "bold", fill: accent)[#exp.company#if "location" in exp and exp.location != "" [ · #exp.location]]])
          v(0.16em)
  ...
  if projects != () and projects.len() > 0 {
    section("Projects", {
      for (i, proj) in projects.enumerate() {
        if i > 0 { v(0.42em) }
        block(stroke: 0.5pt + line, inset: 0.38em, radius: 2pt, width: 100%, [#text(weight: "bold")[#proj.name] #if "description" in proj [— #parse-bold(proj.description)]])
      }
    })
  }
  ...
  if "education" in data and data.education.len() > 0 {
    section("Education", {
      for (i, edu) in data.education.enumerate() {
        if i > 0 { v(0.34em) }
        grid(columns: (1fr, auto), text(weight: "bold")[#edu.degree], text(size: 8pt, fill: muted)[#edu.dates]); v(0.03em); text(size: 8.2pt, fill: accent)[#edu.institution]
      }
    })
  }
