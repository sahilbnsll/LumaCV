...
#let render-editorial(data, variant: "default", theme: "editorial") = {
  ...
  let accent = rgb("#8b3a3a")
  let soft = rgb("#f5efed")
  let stroke-line = rgb("#e5d8d5")
  let white = rgb("#ffffff")

  set document(title: name + " — Resume", author: name, date: none)
  set page(paper: "a4", margin: (x: 1.18cm, y: 1.08cm))
  set text(font: "Libertinus Serif", size: 9.0pt, fill: ink, hyphenate: false, fallback: true)
  set par(leading: 0.56em, spacing: 0.45em, justify: false, linebreaks: "optimized")
  ...
  let section(title, body) = {
    ...
    block(above: 0pt, below: 0.13em, breakable: false, sticky: true, width: 100%, {
      set text(size: 10.5pt, weight: "bold", fill: accent); text(tracking: 0.08em, upper(title)); v(0.10em); line(length: 100%, stroke: 0.55pt + stroke-line)
    })
    body
  }
  align(left)[text(size: 24pt, weight: "bold", fill: accent)[#name]]; if headline != "" { v(0.08em); text(size: 10pt, style: "italic", fill: muted)[#headline] }; v(0.20em); text(size: 8pt)[#contact-text]; v(0.30em); line(length: 100%, stroke: 1.2pt + accent)
    ...
    section("Experience", {
      for (i, exp) in data.experience.enumerate() {
        ...
        block(width: 100%, breakable: false, {
          grid(columns: (1fr, auto), column-gutter: 0.7em, align: (left + top, right + top), text(size: 9.4pt, weight: "bold")[#exp.role], text(size: 8.2pt, fill: muted)[#exp.dates]); v(0.05em); text(size: 8.6pt, style: "italic", fill: accent)[#exp.company#if "location" in exp and exp.location != "" [ · #exp.location]]
          v(0.16em)
  ...
  if projects != () and projects.len() > 0 {
    section("Projects", {
      for (i, proj) in projects.enumerate() {
        if i > 0 { v(0.42em) }
        block(fill: soft, inset: (x: 0.50em, y: 0.36em), radius: 3pt, width: 100%, [#text(weight: "bold")[#proj.name]#if "stack" in proj and proj.stack != "" [ #text(size: 7.9pt, fill: muted)[· #proj.stack]] v(0.10em) #if "description" in proj [#parse-bold(proj.description)]])
      }
    })
  }
  ...
  if "education" in data and data.education.len() > 0 {
    section("Education", {
      for (i, edu) in data.education.enumerate() {
        if i > 0 { v(0.34em) }
        grid(columns: (1fr, auto), column-gutter: 0.7em, text(weight: "bold")[#edu.degree#if "specialization" in edu and edu.specialization != "" [ · #edu.specialization]], text(fill: muted)[#edu.dates]); v(0.04em); text(size: 8.2pt, fill: accent)[#edu.institution#if "gpa" in edu and edu.gpa != "" [ · #edu.gpa]]
      }
    })
  }
  ...
  if "certifications" in data and data.certifications.len() > 0 {
    section("Certifications", {
      for (i, cert) in data.certifications.enumerate() {
        if i > 0 { v(0.22em) }
        text(weight: "bold")[#cert.name]#if "issuer" in cert and cert.issuer != "" [#text(fill: muted)[ · #cert.issuer]]#if "date" in cert and cert.date != "" [#h(1fr)#text(size: 8pt, fill: muted)[#cert.date]]
      }
    })
  }
}
