// ============================================================
// Template: Executive (Executive) — Wide-Margin Leadership Serif
// ============================================================

#import "/lib/utils.typ": parse-bold, clean-link, resolve-variant-field
#import "/lib/theme.typ": font-serif, render-footer

#let render-executive(data, variant: "default", theme: "executive") = {
  let t = (
    ink: rgb("#1f2328"),
    accent: rgb("#8a6d3b"),
    accent-soft: rgb("#faf7f0"),
    border-line: rgb("#e6dcce"),
    muted: rgb("#6b7280"),
    marker: rgb("#8a6d3b"),
    link: rgb("#8a6d3b"),
  ...
  set page(
    paper: "a4",
    margin: (x: 1.40cm, top: 1.15cm, bottom: 1.25cm),
    footer: render-footer(t, name),
  )

  set text(font: font-serif, size: 9.0pt, fill: t.ink, hyphenate: false, fallback: true)
  ...
  // Boardroom Centered Header
  align(center)[
    #text(size: 21pt, weight: "bold", tracking: 0.05em, fill: t.name-fill)[#upper(name)]
    #if headline != "" [
      #v(0.12em)
      #text(size: 9.4pt, style: "italic", fill: t.headline-fill)[#headline]
    ]
    #v(0.20em)
    #line(length: 45%, stroke: 0.6pt + t.accent)
    #v(0.14em)
    ...
    block(
      ...
      breakable: breakable,
      {
        block(
          ...
          width: 100%,
          align(center)[
            #set text(size: 9.4pt, weight: "bold", fill: t.accent, tracking: 0.16em)
            #upper(title)
            #v(0.08em)
            #line(length: 32%, stroke: 0.6pt + t.accent)
          ]
        )
        v(0.18em)
        body
  ...
  let entry-heading(company, role, dates, location) = {
    grid(
      ...
      text(size: 9.2pt, weight: "bold", fill: t.ink)[#role], text(size: 8.2pt, style: "italic", fill: t.muted)[#dates],
    )
    v(0.02em)
    grid(
      columns: (1fr, auto), column-gutter: 0.8em, align: (left + top, right + top),
      text(size: 8.8pt, weight: "bold", fill: t.company-fill)[#company],
      if location != "" { text(size: 8pt, style: "italic", fill: t.muted)[#location] } else { none },
    )
  }
  ...
  for sec in active-order {
    if sec == "summary" {
      if summary != "" {
        section("Executive Summary", parse-bold(summary))
      }
      ...
      if "techStackSummary" in data and data.techStackSummary != none and data.techStackSummary != "" {
        section("Core Tech Stack", block(
          ...
          radius: 2.5pt,
          width: 100%,
          text(size: 8.2pt)[#text(weight: "bold", fill: t.ink)[Core Technologies: ] #parse-bold(data.techStackSummary)]
        ))
      }
    } else if sec == "keyMetrics" {
      if "keyMetrics" in data and data.keyMetrics != none and data.keyMetrics.len() > 0 {
        section("Executive Metrics & Impact", {
      ...
      if projects != () and projects.len() > 0 {
        section("Key Initiatives & Architecture", breakable: false, {
          for (i, proj) in projects.enumerate() {
            ...
            block(width: 100%, breakable: false, {
              ...
              if "stack" in proj and proj.stack != "" {
                v(0.05em)
                text(size: 8pt, fill: t.accent)[Stack: #proj.stack]
              }
        ...
        section("Education", {
          for (i, edu) in data.education.enumerate() {
            ...
            grid(
              ...
              text(size: 9.4pt, weight: "bold", fill: t.ink)[#edu.institution],
              text(size: 8.4pt, style: "italic", fill: t.muted)[#edu.dates],
            )
            v(0.04em)
            grid(
              columns: (1fr, auto),
              [#text(size: 8.5pt, fill: t.ink)[#edu.degree#if "specialization" in edu and edu.specialization != "" [ · #edu.specialization]]],
              if "gpa" in edu and edu.gpa != "" { text(size: 8.4pt, fill: t.muted)[GPA: #edu.gpa] }
            )
          }
        })
      }
      ...
      if "certifications" in data and data.certifications.len() > 0 {
        section("Certifications & Governance", breakable: false, {
          for (i, cert) in data.certifications.enumerate() {
            ...
            grid(
              ...
              text(size: 8.4pt, style: "italic", fill: t.muted)[#if "date" in cert { cert.date }]
            )
          }
        })
      }
      ...
      if not awards-rendered {
        ...
        if items.len() > 0 {
          section("Honors & Board Recognitions", breakable: false, {
            for (i, ach) in items.enumerate() {
              ...
              grid(
                ...
                text(size: 8.4pt, style: "italic", fill: t.muted)[#if "date" in ach { ach.date }]
              )
              if "description" in ach and ach.description != "" {
                v(0.05em)
                text(size: 8.4pt, fill: t.ink)[#parse-bold(ach.description)]
              }
            }
          })
        }
        ...
        section("Open Source Architecture", breakable: false, {
          for (i, item) in data.openSource.enumerate() {
            ...
            grid(
              ...
              text(size: 8.4pt, style: "italic", fill: t.muted)[#if "dates" in item { item.dates }]
            )
            if "bullets" in item and item.bullets.len() > 0 {
              ...
              v(0.05em)
              text(size: 8.4pt, fill: t.ink)[#parse-bold(item.impact)]
            }
          }
        })
      }
        ...
        section("Publications", breakable: false, {
          for (i, pub) in data.publications.enumerate() {
            ...
            grid(
              ...
              text(size: 8.4pt, style: "italic", fill: t.muted)[#if "date" in pub { pub.date }]
            )
            if "description" in pub and pub.description != "" {
              v(0.05em)
              text(size: 8.4pt, fill: t.ink)[#parse-bold(pub.description)]
            }
          }
        })
      }
        ...
        section("Advisory & Leadership", breakable: false, {
          for (i, lead) in data.leadership.enumerate() {
            ...
            grid(
              ...
              text(size: 8.4pt, style: "italic", fill: t.muted)[#if "dates" in lead { lead.dates }]
            )
        ...
        section("Community & Mentorship", breakable: false, {
          for (i, vol) in data.volunteering.enumerate() {
            ...
            grid(
              ...
              text(size: 8.4pt, style: "italic", fill: t.muted)[#if "dates" in vol { vol.dates }]
            )
        ...
        section("Conferences & Keynotes", breakable: false, {
          for (i, conf) in data.conferences.enumerate() {
            ...
            grid(
              ...
              text(size: 8.4pt, style: "italic", fill: t.muted)[#if "date" in conf { conf.date }]
            )
            if "description" in conf and conf.description != "" {
              v(0.05em)
              text(size: 8.4pt, fill: t.ink)[#parse-bold(conf.description)]
            }
          }
        })
      }
      ...
      if "patents" in data and data.patents.len() > 0 {
        section("Patents & Intellectual Property", breakable: false, {
          for (i, pat) in data.patents.enumerate() {
            ...
            [— #title-line #if meta.len() > 0 [ #text(size: 8.4pt, fill: t.muted)[— #meta.join(", ")] ]]
            if "description" in pat and pat.description != "" {
              v(0.05em)
              text(size: 8.4pt, fill: t.ink)[#parse-bold(pat.description)]
            }
          }
        })
      }
