...
#let render-executive(data, variant: "default", theme: "executive") = {
  ...
  let section(title, body) = {
    ...
    block(above: 0pt, below: 0.13em, breakable: false, sticky: true, width: 100%, {
      text(size: 9.5pt, weight: "bold", fill: ink, tracking: 0.12em)[#upper(title)]; v(0.08em); line(length: 100%, stroke: 0.65pt + accent)
    })
    body
  }
    ...
    section("Experience", {
      for (i, exp) in data.experience.enumerate() {
        ...
        block(width: 100%, breakable: false, {
          grid(columns: (1fr, auto), column-gutter: 0.7em, text(size: 9.3pt, weight: "bold")[#exp.company], text(size: 8pt, weight: "bold", fill: accent)[#exp.dates]); v(0.04em); text(size: 8.5pt, style: "italic", fill: muted)[#exp.role#if "location" in exp and exp.location != "" [ · #exp.location]]
          v(0.16em)
  ...
  if projects != () and projects.len() > 0 {
    section("Projects", {
      for (i, proj) in projects.enumerate() {
        if i > 0 { v(0.42em) }
        grid(columns: (auto, 1fr), column-gutter: 0.45em, text(size: 8pt, weight: "bold", fill: white)[#circle(fill: accent, radius: 3pt)[ ]], [#text(weight: "bold")[#proj.name]#if "description" in proj [ — #parse-bold(proj.description)]])
      }
    })
  }
  ...
  if "education" in data and data.education.len() > 0 {
    section("Education", {
      for (i, edu) in data.education.enumerate() {
        if i > 0 { v(0.34em) }
        grid(columns: (1fr, auto), text(weight: "bold")[#edu.institution], text(size: 8pt, fill: muted)[#edu.dates]); v(0.04em); text(size: 8.2pt)[#edu.degree#if "specialization" in edu and edu.specialization != "" [ · #edu.specialization]]
      }
    })
  }
