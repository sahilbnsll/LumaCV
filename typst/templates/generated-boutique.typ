// ============================================================
// Template: Boutique — Refined Editorial Whitespace & Mulberry Serif
// ============================================================
// Design principles:
//  - Elegant centered editorial serif header with mulberry accent
//  - Centered uppercase tracked section headers with anchored ornament rule
//  - Dark plum-charcoal ink, crisp bold item titles, muted plum metadata
...
#let render-boutique(data, variant: "default", theme: "boutique") = {
  let default-palette = (
    ...
    muted: rgb("#756a7a"),
    soft: rgb("#f8f4f9"),
    stroke-line: rgb("#e2d8e4"),
    marker: rgb("#6f4c78"),
    link: rgb("#6f4c78"),
    name-fill: rgb("#2a2030"),
    headline-fill: rgb("#6f4c78"),
    company-fill: rgb("#6f4c78"),
  )
  ...
  set page(paper: pg.paper, margin: (x: 1.30cm, top: 1.35cm, bottom: 1.85cm), footer: render-footer(t, name))
  ...
  set list(indent: 0.90em, body-indent: 0.35em, spacing: 0.36em, marker: (text(fill: accent)[•], text(fill: accent)[–]))
  ...
  align(center)[
    ...
    #if headline != "" [
      #v(0.08em)
      #text(size: 9.4pt, style: "italic", fill: accent)[#headline]
    ]
    #v(0.14em)
    #text(size: 7.9pt, fill: muted)[#contact-items.join(text(fill: muted)[ · ])]
  ]
  v(0.20em)
  ...
  let section(title, body, breakable: true) = {
    ...
    block(width: 100%, breakable: breakable, {
      block(breakable: false, sticky: true, width: 100%, align(center, {
        text(size: 8.8pt, weight: "bold", fill: accent, tracking: 0.14em)[#upper(title)]
        v(0.24em)
        line(length: 34pt, stroke: 1pt + accent)
        v(0.20em)
      }))
      v(0.20em)
      body
  ...
  for sec in active-order {
    if sec == "summary" {
      ...
      if projects != () and projects.len() > 0 {
        section("Curated Projects & Initiatives", {
      ...
      if "additionalInfo" in data and data.additionalInfo != none {
        ...
        if items.len() > 0 {
          section("Additional Information", items.join(text(fill: muted)[ · ]), breakable: false)
        }
      }
