...
#let render-boutique(data, variant: "default", theme: "boutique") = {
  ...
  let section(title, body) = {
    ...
    block(above: 0pt, below: 0.13em, breakable: false, sticky: true, width: 100%, [
      #align(center)[
        #text(size: 9.7pt, weight: "bold", fill: accent, tracking: 0.10em)[#upper(title)]
        #v(0.09em)
        #line(length: 34pt, stroke: 0.9pt + accent)
        #v(0.16em)
      ]
    ])
    body
    ...
    section("Experience", {
      for (i, exp) in data.experience.enumerate() {
        ...
        block(width: 100%, breakable: false, [
          #grid(columns: (1fr, auto), text(size: 9.4pt, weight: "bold")[#exp.company], text(size: 8pt, fill: muted)[#exp.dates])
          #v(0.04em)
          #text(size: 8.8pt, style: "italic", fill: accent)[#exp.role#if "location" in exp and exp.location != "" [ · #exp.location]]
          #v(0.16em)
  ...
  if projects != () and projects.len() > 0 {
    section("Projects", {
      for (i, proj) in projects.enumerate() {
        if i > 0 { v(0.42em) }
        block(fill: soft, inset: (x: 0.45em, y: 0.32em), width: 100%, radius: 7pt, [
          #align(center)[
            #text(weight: "bold", fill: accent)[#proj.name]
            #if "description" in proj and proj.description != "" [\ #v(0.06em)#parse-bold(proj.description)]
          ]
        ])
      }
    })
    ...
    section("Education", {
      for (i, edu) in data.education.enumerate() {
        ...
        align(center)[
          ...
          #v(0.03em)
          #text(size: 8pt, fill: muted)[#edu.dates]
        ]
      }
    })
  }
