// ============================================================
// Template: Ledger — Editorial Ledger-Style Serif Layout
// ============================================================
// Design principles:
//  - Formal serif styling with financial / accounting precision
//  - Double rule header, uppercase section titles with full-width rule
//  - High-density information design with elegant breathing room
...
#let render-ledger(data, variant: "default", theme: "ledger") = {
  let default-palette = (
    ...
    marker: rgb("#2a5c4e"),
    link: rgb("#2a5c4e"),
    name-fill: rgb("#1c2826"),
    headline-fill: rgb("#2a5c4e"),
    company-fill: rgb("#2a5c4e"),
  )
  ...
  let accent-bg = if "accent" in t { t.accent.lighten(94%) } else { rgb("#f2f7f5") }
  let accent-border = if "accent" in t { t.accent.lighten(70%) } else { rgb("#c5ded7") }
  ...
  // Header Block
  align(center)[
    #text(size: 22pt, weight: "bold", tracking: 0.12em, fill: t.name-fill)[#upper(name)]
    #if headline != "" [
      #v(0.12em)
  ...
  v(0.20em)
  line(length: 100%, stroke: 1pt + t.ink)
  v(0.03em)
  line(length: 100%, stroke: 0.4pt + t.accent)

  let section(title, body, breakable: true) = {
    v(1.15em)
    block(width: 100%, breakable: breakable, {
      block(breakable: false, sticky: true, width: 100%, {
        text(font: font-sans, size: 8.8pt, weight: "bold", tracking: 0.14em, fill: t.accent)[#heading(level: 2, outlined: true, bookmarked: true, upper(title))]
        v(0.08em)
        line(length: 100%, stroke: 0.50pt + rgb("#d8dedb"))
      })
      v(0.22em)
      body
    })
