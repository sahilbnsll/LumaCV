...
#let render-terminal(data, variant: "default", theme: "terminal") = {
  ...
  let section(title, body) = {
    ...
    block(above: 0pt, below: 0.13em, breakable: false, sticky: true, width: 100%, {
      text(size: 8.3pt, weight: "bold", fill: accent)["$ " + title]; v(0.05em); line(length: 100%, stroke: 0.45pt + stroke-line)
    })
    body
  }
  block(fill: soft, inset: (x: 0.50em, y: 0.42em), radius: 4pt, [#text(size: 17pt, weight: "bold", fill: accent)[#name]
  v(0.06em)#if headline != "" [#text(size: 8.3pt, fill: ink)[#headline]]
  v(0.08em)#text(size: 7.2pt, fill: muted)[#contact-text]]) v(0.16em)
    ...
    section("Experience", {
      for (i, exp) in data.experience.enumerate() {
        ...
        block(width: 100%, breakable: false, {
          grid(columns: (1fr, auto), text(size: 8.5pt, weight: "bold")[#exp.role] [@" + " " + exp.company], text(size: 7.5pt, fill: muted)[#exp.dates]); if "location" in exp and exp.location != "" { v(0.02em); text(size: 7.5pt, fill: muted)[#exp.location] }
          v(0.16em)
  ...
  if projects != () and projects.len() > 0 {
    section("Projects", {
      for (i, proj) in projects.enumerate() {
        if i > 0 { v(0.42em) }
        grid(columns: (auto, 1fr), column-gutter: 0.35em, text(fill: accent)[>], [#text(weight: "bold")[#proj.name]#if "stack" in proj [ · #text(fill: muted)[#proj.stack]] v(0.04em) #if "description" in proj [#parse-bold(proj.description)]])
      }
    })
  }
  ...
  if "certifications" in data and data.certifications.len() > 0 {
    section("Certifications", {
      for (i, cert) in data.certifications.enumerate() {
        if i > 0 { v(0.22em) }
        [#text(fill: accent)[+] #text(weight: "bold")[#cert.name]#if "issuer" in cert and cert.issuer != "" [#text(fill: muted)[ — #cert.issuer]]]
      }
    })
  }
}
