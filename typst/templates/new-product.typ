...
#let render-product(data, variant: "default", theme: "product") = {
  let t = (
    ...
    marker: rgb("#5b47d1"),
    link: rgb("#5b47d1"),
    banner-bg: rgb("#f5f3ff"),
    card-bg: rgb("#faf5ff"),
    border-line: rgb("#ede9fe"),
  )
  ...
  let contact-items = ()
  if "location" in c and c.location != "" { contact-items.push(c.location) }
  if "email" in c and c.email != "" { contact-items.push(link("mailto:" + c.email)[#c.email]) }
  if "phone" in c and c.phone != "" { contact-items.push(link("tel:" + c.phone.replace(" ", ""))[#c.phone]) }
  if "linkedin" in c and c.linkedin != "" { contact-items.push(link("https://" + clean-link(c.linkedin))[#clean-link(c.linkedin)]) }
  if "github" in c and c.github != "" { contact-items.push(link("https://" + clean-link(c.github))[#clean-link(c.github)]) }
  if "website" in c and c.website != "" { contact-items.push(link("https://" + clean-link(c.website))[#clean-link(c.website)]) }
  ...
  grid(
    ...
    [
      #text(size: 20pt, weight: "bold", fill: t.ink)[#name]
      #if headline != "" [\ #v(0.08em)#text(size: 8.8pt, weight: "medium", fill: t.accent)[#headline]]
    ],
    [
      #align(right + top)[#text(size: 7.6pt, fill: t.muted)[#contact-text]]
    ]
  )
  v(0.16em)
  ...
  let section(title, body, breakable: true) = {
    ...
    block(width: 100%, breakable: breakable, [
      #block(
        fill: t.banner-bg,
        inset: (x: 0.52em, y: 0.28em),
        radius: 2.5pt,
        width: 100%,
        breakable: false,
        sticky: true,
        text(size: 8.6pt, weight: "bold", tracking: 0.08em, fill: t.accent)[#upper(title)]
      )
      #v(0.20em)
      #body
    ])
