...
#let render-statement(data, variant: "default", theme: "statement") = {
  ...
  let section(title, body) = {
    ...
    block(above: 0pt, below: 0.13em, breakable: false, sticky: true, width: 100%, [
      #grid(columns: (auto, 1fr), column-gutter: 0.42em, text(size: 13pt, weight: "bold", fill: accent)[/], text(size: 9.1pt, weight: "bold", tracking: 0.08em, upper(title)))
      #v(0.08em)
  ...
  grid(
    ...
    text(size: 43pt, weight: "bold", fill: accent)[/],
    [
      #text(size: 23pt, weight: "bold", fill: ink)[#name]
      #if headline != "" [\ #v(0.03em)#text(size: 9.2pt, fill: accent)[#headline]]
      #v(0.10em)
  ...
  if projects != () and projects.len() > 0 {
    section("Projects", {
      for (i, proj) in projects.enumerate() {
        if i > 0 { v(0.42em) }
        block(stroke: 0.55pt + stroke-line, inset: (x: 0.44em, y: 0.32em), radius: 3pt, [
          #text(weight: "bold")[#proj.name]
          #if "stack" in proj and proj.stack != "" [ · #text(fill: accent, size: 7.8pt)[#proj.stack]]
          #if "description" in proj and proj.description != "" [ \ #v(0.05em)#parse-bold(proj.description)]
        ])
      }
    })
  }
  ...
  if "certifications" in data and data.certifications.len() > 0 {
    section("Certifications", {
      for (i, cert) in data.certifications.enumerate() {
        if i > 0 { v(0.22em) }
        [#text(weight: "bold")[#cert.name]#if "issuer" in cert and cert.issuer != "" [ #text(fill: muted)[· #cert.issuer]]]
      }
    })
  }
}
