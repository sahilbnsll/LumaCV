...
#let render-matrix(data, variant: "default", theme: "matrix") = {
  ...
  grid(
    ...
    [
      ...
      #if headline != "" [\ #v(0.05em)#text(size: 8.5pt, fill: accent)[#headline]]
    ],
    [
      #align(right)[#text(size: 7.4pt, fill: muted)[#contact-text]]
    ]
  )
  v(0.13em)
    ...
    section("Experience", {
      for (i, exp) in data.experience.enumerate() {
        ...
        block(width: 100%, breakable: false, [
          #grid(
            columns: (1fr, auto),
            [#text(size: 8.9pt, weight: "bold")[#exp.role] #text(fill: accent)[·] #exp.company],
  ...
  if projects != () and projects.len() > 0 {
    section("Projects", {
      for (i, proj) in projects.enumerate() {
        if i > 0 { v(0.42em) }
        block(fill: soft, inset: 0.36em, radius: 3pt, [
          #text(weight: "bold", fill: accent)[#proj.name]
  ...
  if "education" in data and data.education.len() > 0 {
    section("Education", {
      for (i, edu) in data.education.enumerate() {
        if i > 0 { v(0.34em) }
        grid(columns: (1fr, auto), text(weight: "bold")[#edu.institution], text(size: 7.8pt, fill: muted)[#edu.dates])
        v(0.03em)
        [#text(size: 8pt)[#edu.degree#if "specialization" in edu and edu.specialization != "" [ · #edu.specialization]]]
      }
    })
  }
  ...
  if "certifications" in data and data.certifications.len() > 0 {
    section("Certifications", {
      for (i, cert) in data.certifications.enumerate() {
        if i > 0 { v(0.22em) }
        block(fill: soft, inset: 0.28em, radius: 2pt, [
