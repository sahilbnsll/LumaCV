// ============================================================
// Template: Azurill (Stackline)
// ============================================================
// Design principles:
//  - High-density modern engineering & technical layout
//  - Cobalt/indigo accents with subtle tag capsules
//  - Direct, fast visual scanning with tabular contact grid
//  - Explicit support for links, dates, and bulleted results
...
#let render-azurill(data, variant: "default", theme: "azurill") = {
  let t = if type(theme) == dictionary { theme } else if type(theme) == str { colors.at(theme, default: colors.azurill) } else { colors.azurill }
  ...
  set page(
    paper: pg.paper,
    margin: (x: 1.35cm, top: 1.30cm, bottom: 1.50cm),
  ...
  // -------------------------------------------------------------------------
  // Header: Name + Headline + Modern Contact Row
  // -------------------------------------------------------------------------
  grid(
    columns: (1fr, auto),
    align: (left + top, right + top),
    [
      #text(size: 20pt, weight: "bold", fill: t.accent)[#name]
      #if headline != "" [
        #v(0.14em)
        #text(size: 9.5pt, weight: "bold", fill: t.ink)[#headline]
      ]
    ],
    align(right + top)[
      #set text(size: 8.0pt)
      #{
        ...
        let line1 = ()
        if "email" in c and c.email != "" { line1.push(link("mailto:" + c.email)[#c.email]) }
        if "phone" in c and c.phone != "" { line1.push(link("tel:" + c.phone.replace(" ", ""))[#c.phone]) }
        if "location" in c and c.location != "" { line1.push(c.location) }
        if line1.len() > 0 { lines.push(line1.join("  |  ")) }
        ...
        if line2.len() > 0 { lines.push(line2.join("  |  ")) }

        lines.join(linebreak() + v(0.14em))
      }
    ]
  )
  ...
  let section(title, body, breakable: false) = {
    ...
    block(width: 100%, breakable: false, sticky: true, {
      rect(
        ...
        width: 100%,
        [
          #text(size: 8.8pt, weight: "bold", fill: t.accent, tracking: 0.08em)[#upper(title)]
        ]
      )
    })
    v(0.48em)
      ...
      if "education" in data and data.education.len() > 0 {
        section("Education", breakable: false, {
          for (i, edu) in data.education.enumerate() {
            if i > 0 { v(0.48em) }
            block(width: 100%, breakable: false, {
              let deg = edu.degree + if "specialization" in edu and edu.specialization != "" [ · #edu.specialization] else []
              entry-title(
