...
#let render-nordic(data, variant: "default", theme: "nordic") = {
  ...
  let section(title, body) = {
    ...
    block(above: 0pt, below: 0.13em, breakable: false, sticky: true, width: 100%, {
      text(size: 9pt, weight: "bold", fill: accent, tracking: 0.17em)[#upper(title)]; v(0.11em); line(length: 34pt, stroke: 1.4pt + accent); v(0.18em)
    })
    body
  }
  align(left)[text(size: 23pt, weight: "bold", fill: ink)[#name]]; v(0.05em); if headline != "" { text(size: 9.5pt, fill: accent)[#headline] }; v(0.20em); text(size: 8pt, fill: muted)[#contact-text]; v(0.30em); line(length: 100%, stroke: 0.8pt + stroke-line)
    ...
    section("Experience", {
      for (i, exp) in data.experience.enumerate() {
        ...
        block(width: 100%, breakable: false, {
          grid(columns: (1fr, auto), column-gutter: 0.6em, text(size: 9.2pt, weight: "bold")[#exp.role], text(size: 8pt, fill: muted)[#exp.dates]); v(0.03em); text(size: 8.2pt, weight: "bold", fill: accent)[#exp.company]#if "location" in exp and exp.location != "" [#text(fill: muted)[ · #exp.location]]
          v(0.16em)
  ...
  if projects != () and projects.len() > 0 {
    section("Projects", {
      for (i, proj) in projects.enumerate() {
        if i > 0 { v(0.42em) }
        block(fill: soft, inset: (x: 0.52em, y: 0.34em), radius: 6pt, width: 100%, [#text(size: 8.8pt, weight: "bold")[#proj.name] #if "stack" in proj and proj.stack != "" [#text(size: 7.7pt, fill: muted)[ · #proj.stack]] v(0.08em) #if "description" in proj [#parse-bold(proj.description)]])
      }
    })
  }
  ...
  if "education" in data and data.education.len() > 0 {
    section("Education", {
      for (i, edu) in data.education.enumerate() {
        if i > 0 { v(0.34em) }
        grid(columns: (1fr, auto), text(weight: "bold")[#edu.degree], text(size: 8pt, fill: muted)[#edu.dates]); v(0.05em); text(size: 8.2pt, fill: accent)[#edu.institution]
      }
    })
  }
