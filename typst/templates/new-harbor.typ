// ============================================================
// Template: Harbor — Maritime Coastal Modern Layout
// ============================================================
// Design principles:
//  - Deep navy ink, nautical slate/teal accent, coastal serenity
//  - Clean block headers with vertical accent indicator mark
//  - Balanced spacious metrics and two-column capability
...
#let render-harbor(data, variant: "default", theme: "harbor") = {
  let default-palette = (
    ...
    marker: rgb("#1d637d"),
    link: rgb("#1d637d"),
    name-fill: rgb("#1b2838"),
    headline-fill: rgb("#1d637d"),
    company-fill: rgb("#1d637d"),
  )
  ...
  let accent-bg = if "accent" in t { t.accent.lighten(94%) } else { rgb("#f0f6f9") }
  let accent-border = if "accent" in t { t.accent.lighten(70%) } else { rgb("#b8d6e3") }

  set document(title: if headline != "" { name + " — " + headline } else { name + " — Resume" }, author: name, date: none)
  set page(paper: pg.paper, margin: (x: 1.35cm, y: 1.25cm), footer: render-footer(t, name))
    ...
    block(width: 100%, breakable: breakable, {
      block(breakable: false, sticky: true, width: 100%, {
        grid(
          ...
          rect(width: 3.5pt, height: 10.5pt, fill: t.accent, radius: 1pt),
          [
            #text(font: font-sans, size: 8.8pt, weight: "bold", tracking: 0.10em, fill: t.accent)[#heading(level: 2, outlined: true, bookmarked: true, upper(title))]
          ]
        )
        v(0.08em)
        line(length: 100%, stroke: 0.50pt + rgb("#d6e1e5"))
      })
      v(0.20em)
      body
    })
  ...
  for sec in active-order {
    if sec == "summary" {
      ...
      if "techStackSummary" in data and data.techStackSummary != none and data.techStackSummary != "" {
        section("Platform & Technical Stack", block(
          fill: accent-bg,
          stroke: 0.5pt + accent-border,
          inset: (x: 7pt, y: 4.5pt),
          radius: 2pt,
          width: 100%,
          text(size: 8.2pt)[#text(weight: "bold", fill: t.company-fill)[Architecture & Tools:] #parse-bold(data.techStackSummary)]
        ))
      }
    } else if sec == "keyMetrics" {
      if "keyMetrics" in data and data.keyMetrics != none and data.keyMetrics.len() > 0 {
        section("Scale & Reliability Metrics", {
      ...
      if skills != () and skills.len() > 0 {
        section("Core Capabilities & Technologies", {
          for cat in skills {
        ...
        section("Professional Experience", {
          for (i, exp) in data.experience.enumerate() {
            ...
            block(width: 100%, breakable: false, {
              ...
              if "location" in exp and exp.location != "" [#text(size: 7.8pt, style: "italic", fill: t.muted)[ · #exp.location]]
              let all-bullets = ()
          ...
          for (i, edu) in data.education.enumerate() {
            ...
            block(width: 100%, breakable: false, {
              grid(
                ...
                text(size: 8.0pt, style: "italic", fill: t.muted)[#edu.dates],
              )
              v(0.02em)
              grid(
                columns: (1fr, auto),
      ...
      if projects != () and projects.len() > 0 {
        section("Engineering Projects & Systems", breakable: false, {
          for (i, proj) in projects.enumerate() {
            ...
            block(width: 100%, breakable: false, {
              grid(
                ...
                if "dates" in proj and proj.dates != "" { text(size: 8.0pt, style: "italic", fill: t.muted)[#proj.dates] }
              )
