...
const templateOptions: Array<{ id: TemplateType; name: string; style: string }> = [
    { id: 'modern', name: 'Vector', style: 'Clean Sans-Serif' },
    { id: 'classic', name: 'Heritage', style: 'Ivy League Serif' },
    { id: 'engineering', name: 'Platform', style: 'High-Density Technical' },
    { id: 'compact', name: 'Compact', style: 'Space-Optimized' },
    { id: 'two_column', name: 'Duplex', style: 'Asymmetric Sidebar' },
    { id: 'ats_safe', name: 'Clearance', style: 'Linear Pure Text' },
];
    ...
    const explainableBullets = useMemo(() => {
        return [
            ...
            },
            {
                ...
                original: 'Built CI/CD pipelines with policy checks, artifact promotion, and automated rollback paths.',
                tailored: 'Built CI/CD pipelines with policy checks and automated rollback paths supporting systems processing millions of events per day, reducing median deployment lead time by 52%.',
                reason: 'Quantified an existing metric (52% lead-time reduction) and aligned scale language with the JD\'s "millions of financial events per day."',
            }
        ];
    }, []);

    const [demoMenuOpen, setDemoMenuOpen] = useState(false);
    const handleDownloadDemoFormat = async (fmt: ExportFormatType) => {
        ...
        await exportResume({
            ...
            typstCode,
            customFilename: `alex-morgan-stripe-sample-${selectedTemplate}`,
        });
    };

    const handleDownloadDemoPdf = () => handleDownloadDemoFormat('pdf');
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-border/50 pb-6 mb-8">
                    <div>
                        ...
                        <p className="mt-1 text-xs sm:text-sm text-muted-foreground">
                            Target Role: <strong className="text-foreground">Senior Full Stack Engineer @ Stripe</strong> • Candidate: Alex Morgan
                        </p>
                    </div>
                    ...
                    <div className="lg:col-span-6 space-y-4">
                        ...
                        >
                            ...
                            >
                                ...
                                {/* Subtle Overlay with multi-format export */}
                                <div className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity bg-background/90 backdrop-blur-md px-2.5 py-1.5 rounded-xl border border-border/80 shadow-md text-xs flex items-center gap-2">
                                    <span className="font-mono text-[10px] text-muted-foreground hidden sm:inline">Export Sample:</span>
                                    <button
                                        type="button"
                                        onClick={handleDownloadDemoPdf}
                                        className="font-medium text-[11px] text-primary hover:underline flex items-center gap-1 cursor-pointer"
                                    ...
                                    <div className="relative">
                                        <button
                                            ...
                                            title="Other formats (Word, Markdown, JSON, Typst)"
                                        >
                                            <ChevronDown className="h-3 w-3" />
                                        </button>
                                        {demoMenuOpen && (
                                            <div className="absolute right-0 bottom-7 z-50 p-1.5 rounded-xl border border-border/80 bg-popover/95 backdrop-blur-md shadow-2xl w-48 text-xs text-foreground space-y-0.5">
                                                <button
                                                    type="button"
                                                    onClick={() => handleDownloadDemoFormat('pdf')}
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => handleDownloadDemoFormat('docx')}
                                                    className="flex w-full items-center justify-between px-2 py-1 rounded-lg hover:bg-muted font-medium text-left"
                                                >
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => handleDownloadDemoFormat('md')}
                                                    className="flex w-full items-center justify-between px-2 py-1 rounded-lg hover:bg-muted font-medium text-left"
                                                >
                                                    <span>Markdown</span>
                                                    <span className="text-[10px] text-emerald-500 font-mono">.md</span>
                                                </button>
                                                <button
                                                    type="button"
                                                    onClick={() => handleDownloadDemoFormat('json')}
                                                    className="flex w-full items-center justify-between px-2 py-1 rounded-lg hover:bg-muted font-medium text-left"
                                                >
                                                    <span>JSON Data</span>
                                                    <span className="text-[10px] text-blue-500 font-mono">.json</span>
                                                </button>
                                                <button
                                                    type="button"
                                                    onClick={() => handleDownloadDemoFormat('typ')}
                                                    className="flex w-full items-center justify-between px-2 py-1 rounded-lg hover:bg-muted font-medium text-left"
                                                >
                                                    <span>Typst Source</span>
                                                    <span className="text-[10px] text-purple-500 font-mono">.typ</span>
                                                </button>
                                            </div>
                                        )}
                                    </div>
