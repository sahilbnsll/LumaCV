...
import {
    ArrowLeft, Check, Heart, Copy, CheckCheck, Wallet, Zap, Server,
...
export default function BillingPage() {
    ...
    return (
        <div className="min-h-screen bg-transparent text-foreground antialiased selection:bg-primary/20 selection:text-primary flex flex-col justify-between">
            <AppHeader />

            <main id="main-content" className="mx-auto max-w-5xl px-4 sm:px-6 py-10 flex-1 w-full space-y-9">
                {/* Back Link */}
                <div>
                    <Button asChild variant="ghost" size="sm" className="h-8 text-xs text-muted-foreground hover:text-foreground gap-1.5 -ml-2 rounded-lg transition-colors">
                        <Link href="/dashboard">
                ...
                <div className="border-b border-border/60 pb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="space-y-1">
                        ...
                        <p className="text-xs sm:text-sm text-muted-foreground max-w-2xl leading-relaxed">
                            LumaCV is completely free for all job applicants. Support our serverless Typst infrastructure and open-source development directly.
                        </p>
                    </div>
                    <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/50 dark:bg-card px-3.5 py-1.5 rounded-full border border-border/70 dark:border-white/10 shrink-0 shadow-2xs">
                        <span className="inline-block h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                        <span className="font-semibold text-foreground">All 52 Templates Free</span>
                    </div>
                </div>

                {/* Active Plan Overview */}
                <div className="relative overflow-hidden rounded-3xl border border-border/80 dark:border-white/10 bg-card p-6 sm:p-8 shadow-sm">
                    {/* Subtle ambient gradient mesh */}
                    <div className="pointer-events-none absolute -right-20 -top-20 h-64 w-64 rounded-full bg-primary/10 dark:bg-primary/15 blur-3xl" />
                    ...
                    <div className="relative z-10 mt-6 pt-6 border-t border-border/60 grid grid-cols-2 sm:grid-cols-3 gap-3.5">
                        {[
                ...
                {/* ── 3 Unified Support Methods (Equal Prominence & Balanced Grid) ── */}
                ...
                <div className="rounded-3xl border border-border/80 dark:border-white/10 bg-card p-6 sm:p-7 shadow-xs space-y-4">
                    <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-1">
                        {[
                            ...
                            <div key={i} className="rounded-2xl border border-border/70 dark:border-white/10 bg-muted/20 dark:bg-muted/10 p-4 space-y-2 hover:border-border transition-colors">
